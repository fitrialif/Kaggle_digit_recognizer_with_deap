...
# memory use for 2080ti with 11G RAM
MEMORY_FRACTION = 0.45 # for 1 job
MEMORY_FRACTION = 0.271 # for 2 jobs (every 100th evaluation produces OOM)
MEMORY_FRACTION = 0.185 # for 3 jobs (unstable every 10th evaluation/training produces OOM)
...
def train_individual_fast(individual, train_dataset, valid_dataset, test_dataset,
    ...
    max_time = 10
    accuracy = train_individual2(individual, max_time, train_dataset, valid_dataset, test_dataset,
                                train_labels, valid_labels, test_labels)
    return accuracy
...
def train_individual_normal(individual, train_dataset, valid_dataset, test_dataset,
                            train_labels, valid_labels, test_labels):
    max_time = 40
    accuracy = train_individual(individual, max_time, train_dataset, valid_dataset, test_dataset,
                                train_labels, valid_labels, test_labels)
    return accuracy
...
def train_individual(individual, max_time, train_dataset, valid_dataset, test_dataset,
    ...
    batch_size, learning_rate, standard_dev, receptive_field_1, filters_1, receptive_field_2, \
        filters_2, activation, _, uuid, _, _ = [x for x in individual]
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        save_path = saver.save(sess, '/tmp/saver_' + uuid + '.ckpt')
...
# this is main evaluation function - variation, it will check if there is a model for the
# individual and if yes it will reload it and continue training for the specified time
# it is decorated by two small functions above
def train_individual2(individual, max_time, train_dataset, valid_dataset, test_dataset,
                     train_labels, valid_labels, test_labels):
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 0.438
    # sess = tf.Session(config=config)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        # try restore session
        try:
            save_path = '/tmp/saver_' + uuid + '.ckpt'
            saver.restore(sess, save_path)
            #print("checkpoint accessible - restoring model successful")
        except ValueError:
            print("checkpoint / model not accessible  - restoring model failed")
...
# from protobuf
def evaluate_individual(individual, test_dataset, test_labels):
    # print(individual)
...
# this function is used exclusively to evaluate individual (get his fitness) by using test data and model extracted
# from protobuf
def evaluate_individual_2(individual, test_dataset, test_labels):
    # print(individual)
    tf.compat.v1.reset_default_graph()
    ...
    # define saver
    saver = tf.compat.v1.train.Saver()
    #saver2 = tf.compat.v1.train.Saver.from_proto(protobuf)
    save_path = '/tmp/saver_' + uuid + '.ckpt'
